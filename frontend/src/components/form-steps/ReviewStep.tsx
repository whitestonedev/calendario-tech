        ...
        <CardContent className="pt-6 space-y-4">
          <div>
            ...
            <dl className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <div>
                <dt className="text-sm font-medium text-gray-500">{t('event.organization')}</dt>
                <dd>{data.organization_name}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">{t('event.event')}</dt>
                <dd>{data.event_name}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">{t('event.edition')}</dt>
                <dd>{data.event_edition}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">{t('form.language')}</dt>
          ...
          <div>
            ...
            <dl className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <div>
                <dt className="text-sm font-medium text-gray-500">{t('event.start')}</dt>
                <dd>
                  {formatDate(data.start_date)} às {data.start_time}
                </dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">{t('event.end')}</dt>
                <dd>
                  {formatDate(data.end_date)} às {data.end_time}
                </dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">{t('event.format')}</dt>
                <dd>{data.online ? t('event.online') : t('event.inPerson')}</dd>
              </div>
              {!data.online && (
                <div className="col-span-2">
                  <dt className="text-sm font-medium text-gray-500">{t('event.addressLabel')}</dt>
                  <dd>{data.address}</dd>
                </div>
              )}
            </dl>
          ...
          <div>
            ...
            <dl className="grid grid-cols-1 gap-2">
              <div>
                <dt className="text-sm font-medium text-gray-500">{t('event.link')}</dt>
                <dd className="break-all">{data.event_link}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">{t('event.cost')}</dt>
                <dd>{data.cost}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">{t('event.description')}</dt>
                <dd>{data.short_description}</dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">{t('event.banner')}</dt>
                <dd className="break-all">{data.banner_link || t('event.notProvided')}</dd>
              </div>
            </dl>
          </div>
